...
The site's mark: a "V" (for Vivek) routed as two right-angle PCB traces
meeting at a shared via, rather than a drawn letterform. Chosen over two
earlier rounds of concepts (a plain geometric V mark, then a set of three
alternatives) as the one that's both personal (keeps the initial) and
...
from this folder; still in git history if ever needed).
